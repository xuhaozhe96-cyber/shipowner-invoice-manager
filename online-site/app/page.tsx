...
import { formatDate, isCosco, splitContainers } from "../lib/utils";
...
export default async function Home() {
  ...
  const vessels = buildVessels(invoices).filter((group) => !archived.has(group.vessel));
  return (
    <main>
      ...
      <section className="card" id="current">
        ...
        {vessels.length ? <div className="tableWrap"><table>
          <thead><tr><th>ETA</th><th>船舶</th><th>船东</th><th>集装箱号（按船东）</th><th>账单</th><th>COSCO 索账</th><th>到港状态</th><th></th></tr></thead>
          <tbody>{vessels.map((vessel) => <tr key={vessel.vessel}>
            ...
            <td className="containerSummary">{vessel.owners.map((owner) => <div key={owner.name}><strong>{owner.name || "待补充船东"}：</strong>{owner.containers.join(", ") || "待补充"}</div>)}</td>
            <td><strong>{vessel.invoices.length}</strong></td>
            <td>{vessel.coscoCount ? `${vessel.coscoDrafts}/${vessel.coscoCount}` : "无需索要"}</td>
...
function buildVessels(invoices: InvoiceRow[]) {
  const map = new Map<string, { vessel: string; eta: string; invoices: InvoiceRow[]; owners: { name: string; containers: string[] }[]; coscoCount: number; coscoDrafts: number }>();
  for (const invoice of invoices) {
    const key = invoice.vessel_name || "待补充船名";
    let group = map.get(key);
    if (!group) { group = { vessel: key, eta: invoice.eta, invoices: [], owners: [], coscoCount: 0, coscoDrafts: 0 }; map.set(key, group); }
    group.invoices.push(invoice);
    if (!group.eta && invoice.eta) group.eta = invoice.eta;
    let owner = group.owners.find((item) => item.name === invoice.owner_name);
    if (!owner) { owner = { name: invoice.owner_name, containers: [] }; group.owners.push(owner); if (isCosco(invoice.owner_name)) group.coscoCount += 1; }
